...
Detected topics are sorted by **longest keyword hit**, so `minimum wage` ranks as government-intervention instead of profit-exploitation's shorter `wage`. Child families (household-capture, energy-utilities, supply-trade, planning-desk, statute-desk) are first-class intents.

The crusher export bar copies a **Hearing Brief**: every matched claim, in rank order, steelman first, with two sources and a deep link. One match is a hearing of one.
...
`HybridClaimRetrievalBackend` merges all three with weighted scores. `ClaimRanker` re-ranks using topic **family** match (parent analyzer id vs child `topicId` via `topicPath`), phrase boosts, tag overlap, and per-phrase claim ID hints. Off-family claims are penalized; child nodes under a detected parent are not.

`SearchService` re-ranks FTS hits with the same phrase/tag precision as the fuzzy fallback, so native SQLite order is not the last word.
...
- `test/crusher_test.dart` - unit tests for analyzer, history, service (longest-keyword intent)
- `test/claim_ranker_test.dart` - topic-family match; child topicId is not penalized
- `test/crusher_real_world_test.dart` - real-world leftist arguments with claim ID assertions (incl. wave8)
