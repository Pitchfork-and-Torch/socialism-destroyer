...
Detected topics are sorted by **longest keyword hit**, so `minimum wage` ranks as government-intervention instead of profit-exploitation's shorter `wage`. Child families (household-capture, energy-utilities, supply-trade) are first-class intents.
