      ...
      child: Wrap(
        ...
        children: [
          _Btn(
            ...
            onPressed: () async {
              final claims =
                  result.matchedClaims.map((m) => m.claim).toList();
              if (claims.isEmpty) {
                if (context.mounted) {
                  await _snack(context, 'No curated claims to brief');
                }
                return;
              }
              await Clipboard.setData(
                ClipboardData(
                  text: ClaimExportService.toHearingBrief(claims),
                ),
              );
              if (context.mounted) {
                await _snack(
                  context,
                  'Hearing Brief copied - ${claims.length} claims, steelman first',
                );
              }
            },
          ),
