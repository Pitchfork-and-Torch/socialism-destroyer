...
/// Match cursor that cannot read past the hit list (query shrinks, index stays).
int bookSearchShownIndex(int index, int length) {
...
class BookSearchBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final shown = bookSearchShownIndex(activeMatchIndex, matches.length);
    final countLabel = hasMatches
        ? 'Match ${shown + 1} of ${matches.length}'
        : 'No matches';
            ...
            if (controller.text.length >= 2) ...[
              ...
              Semantics(
                ...
                child: ExcludeSemantics(
                  child: Text(
                    hasMatches ? '${shown + 1}/${matches.length}' : '0',
