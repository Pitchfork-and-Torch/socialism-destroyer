...
class ClaimDetailToolbar extends ConsumerWidget {
      ...
      child: Row(
        ...
        children: [
          ...
          _ActionIcon(
            ...
            tooltip: 'Copy ready post',
            color: context.sd.accentGold,
            onPressed: () => _copyReadyPost(context, claim),
          ),
          _ActionIcon(
  ...
  Future<void> _copyReadyPost(BuildContext context, Claim claim) async {
    final post = ClaimExportService.toReadyPost(claim);
    await Clipboard.setData(ClipboardData(text: post));
    if (!context.mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Ready post copied - steelman first, with the claim link'),
