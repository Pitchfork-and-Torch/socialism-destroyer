...
/// True when [raw] can be handed to [jsonDecode] as a knowledge document.
///
/// The Pages SPA fallback answers missing `/knowledge/` files with
/// `index.html` and a 200. That body must not be stored or parsed as JSON.
bool looksLikeJsonDocument(String raw) {
...
class KnowledgeOverlayStore {
  ...
  Future<void> _dropBadHiveValue(String key) async {
    final box = _hiveBox;
  ...
  Future<String?> readOverlayAsset(String bundlePath) async {
    if (_usesHive) {
      final key = '$_hiveAssetPrefix${assetKeyFromBundlePath(bundlePath)}';
      final raw = _hiveBox?.get(key);
      if (raw == null) return null;
      if (!looksLikeJsonDocument(raw)) {
        await _dropBadHiveValue(key);
        return null;
      }
      return raw;
    }
    ...
    if (!looksLikeJsonDocument(raw)) return null;
    return raw;
  }
  ...
  Future<KnowledgeManifest?> loadOverlayManifest() async {
    ...
    if (raw == null || !looksLikeJsonDocument(raw)) {
      if (raw != null) await _dropBadHiveValue(_hiveManifestKey);
      return null;
    }
    try {
      return KnowledgeManifest.fromJson(jsonDecode(raw) as Map<String, dynamic>);
    } catch (_) {
      await _dropBadHiveValue(_hiveManifestKey);
      return null;
    }
  }
