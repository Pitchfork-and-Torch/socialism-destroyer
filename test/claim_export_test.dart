...
void main() {
  ...
  test('toReadyPost is steelman-first and links the claim', () {
    final post = ClaimExportService.toReadyPost(sample);
    expect(post, startsWith(sample.title));
    expect(post, contains('Steelman: ${sample.socialistClaimText}'));
    expect(post.indexOf('Steelman:'), lessThan(post.indexOf('https://')));
    expect(post, contains(ClaimExportService.shareUrl(sample.id)));
    expect(post.contains('\u2014'), isFalse);
  });
  ...
  test('toHearingBrief keeps rank order and steelman before rebuttal', () {
    final second = Claim(
      id: 'second-claim',
      topicId: 'planning-desk',
      topicPath: '/government-intervention/planning',
      title: 'Second Hearing Claim',
      socialistClaimText: 'Second steelman stays second.',
      executiveSummary: 'Second rebuttal.',
      evidenceBullets: const ['Second evidence'],
      ...
      relatedClaimIds: const [],
      tags: const ['planning-desk'],
      schemaVersion: 2,
      revision: 1,
      updatedAt: '2026-09-21T23:40:00Z',
      embeddingText: 'second',
      searchText: 'second hearing',
      kbVersion: '3.19.0',
    );
    final brief = ClaimExportService.toHearingBrief([sample, second, sample]);
    expect(brief, contains('HEARING BRIEF'));
    expect(brief, contains('2 matched claims'));
    expect(brief.indexOf(sample.title), lessThan(brief.indexOf(second.title)));
    expect(brief.indexOf('STEELMAN'), lessThan(brief.indexOf('REBUTTAL')));
    expect(brief.split('STEELMAN').length, 3);
  });
