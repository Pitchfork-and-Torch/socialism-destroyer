...
void main() {
  ...
  group('Real-world Argument Crusher', () {
    ...
    test('11 - Medicare for All pays for itself', () async {
    ...
    test('14 - cancel all federal student debt', () async {
      const input =
      ...
      expect(
        result.matchedClaimIds,
        contains('cancel-all-student-debt'),
        reason: 'Got ${result.matchedClaimIds}',
      );
      expect(result.primaryClaim?.id, 'cancel-all-student-debt');
    });

    test('16 - social wealth fund owns a slice of every firm', () async {
      const input =
          'a social wealth fund should own a slice of every large firm and pay a resident dividend';
      final result = await crusher.crush(input);

      expectQualityResponse(result);
      expect(result.analysis.detectedTopicIds.first, 'planning-desk');
      expect(
        result.matchedClaimIds,
        contains('social-wealth-fund-owns-the-market'),
        reason: 'Got ${result.matchedClaimIds}',
      );
      expect(result.primaryClaim?.id, 'social-wealth-fund-owns-the-market');
    });

    test('17 - tariff wall says foreigners pay', () async {
      const input =
          'a broad tariff wall makes foreigners pay to rebuild industry and wages';
      final result = await crusher.crush(input);

      expectQualityResponse(result);
      expect(
        result.matchedClaimIds,
        contains('tariff-wall-rebuilds-wages'),
        reason: 'Got ${result.matchedClaimIds}',
      );
      expect(result.primaryClaim?.id, 'tariff-wall-rebuilds-wages');
    });

    test('15 - ban stock buybacks so profits go to wages', () async {
      const input =
      ...
      expect(
        result.matchedClaimIds.any(
          (id) => id == 'ban-stock-buybacks' || id == 'stock-buybacks-are-theft',
        ),
        isTrue,
        reason: 'Got ${result.matchedClaimIds}',
      );
