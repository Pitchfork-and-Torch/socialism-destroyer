...
void main() {
  ...
  group('KnowledgeVersion', () {
    test('compares semantic versions', () {
      ...
      // Multi-digit minors must not string-sort: 3.13 > 3.9
      expect(KnowledgeVersion.compare('3.13.0', '3.9.0'), greaterThan(0));
      expect(KnowledgeVersion.compare('3.9.0', '3.13.0'), lessThan(0));
    });
  });

  group('ChangelogDocument', () {
    test('sorts entries by semver not lexicographic string', () {
      final doc = ChangelogDocument.fromJson({
        ...
        'entries': [
          {
            ...
            'date': '2026-07-26',
            'title': 'Old wave',
            'changes': ['8 PD steelman claims'],
          },
          {
            'version': '3.13.0',
            'date': '2026-08-01',
            'title': 'Latest',
            'changes': ['6 new high-intent claims'],
          },
          {
            'version': '3.12.0',
            'date': '2026-08-01',
            'title': 'Mid',
            'changes': ['Source enrichment'],
          },
        ],
      });
      expect(doc.entries.map((e) => e.version).toList(), [
        '3.13.0',
        '3.12.0',
        '3.9.0',
      ]);
      expect(doc.entries.first.changes.first, contains('high-intent'));
    });
  ...
  group('KnowledgeOverlayStore', () {
    ...
    test('html spa fallback is not a json document', () {
      expect(looksLikeJsonDocument('<!DOCTYPE html><html></html>'), isFalse);
      expect(looksLikeJsonDocument('{"kbVersion":"3.19.0"}'), isTrue);
    });
    ...
    test('html overlay is ignored so the bundled json is used', () async {
      const assetPath = 'assets/data/v2/topics.json';
      await overlay.writeAsset(assetPath, '<!DOCTYPE html><html></html>');
      expect(await overlay.readOverlayAsset(assetPath), isNull);
    });
  ...
  group('KnowledgeSyncService', () {
    ...
    test('loads bundled changelog', () async {
      ...
      expect(changelog.currentVersion, '3.19.0');
